...
Five publication-grade result figures from the 24-month run sit under `docs/assets/results/` and are embedded inline below in the headline section. Each PNG is rendered at 600 DPI from the master-eval CSV outputs.
...
<p align="center">
  <img src="docs/assets/results/01_top20_leaderboard.png" alt="Top 20 unified leaderboard, 24-month run" width="92%"/>
  <br/><sub>Top 20 cells across rule-based and ML stacks, sorted by composite score. Bar colour encodes grade (A green, B light-green, C yellow, D orange, F red). Three cells reach B or above; everything past rank five sits in C territory.</sub>
</p>
...
<p align="center">
  <img src="docs/assets/results/04_dm_forest_plot.png" alt="Diebold-Mariano forest plot, 24-month run" width="78%"/>
  <br/><sub>DM forest plot. Each row is one comparison (pair on top, comparison label below). Light-green row backgrounds mark p < 0.05; only two of the in-domain-versus-transfer probes clear that bar. Most rows sit far from significance.</sub>
</p>
...
<p align="center">
  <img src="docs/assets/results/05_best_per_pair.png" alt="Best configuration per pair" width="78%"/>
  <br/><sub>Best configuration per pair. Three pairs reach composite >= 70 (USDCHF, EURUSD, USDCAD). One pair (AUDUSD) has no profitable configuration anywhere in the universe of evaluated cells.</sub>
</p>
...
The cross-session transfer behaviour is what RQ1 was built to measure. The two grids below are the per-pair 4-by-4 net-Sharpe matrices for each model class, rows are training session and columns are evaluation session. Diagonal cells are in-domain, off-diagonal cells are cross-session transfer.

<p align="center">
  <img src="docs/assets/results/02_lr_transfer_grid.png" alt="LR cross-session transfer grid" width="92%"/>
  <br/><sub>Logistic Regression. Each panel uses its own diverging colour scale so small-range pairs are not visually flattened by NZDUSD's larger range. Hatched panels mean no trades in the window.</sub>
</p>

<p align="center">
  <img src="docs/assets/results/03_lstm_transfer_grid.png" alt="LSTM cross-session transfer grid" width="92%"/>
  <br/><sub>Multi-scale LSTM. The standout cell is USDCHF NY-trained, Asia-evaluated (composite 84.7, the only A-grade in the run).</sub>
</p>
