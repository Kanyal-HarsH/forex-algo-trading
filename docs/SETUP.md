...
Expected console output (abridged, banner separators trimmed):
...
FX ALGO MASTER EVAL  |  run_id=YYYYMMDD_HHMMSS
  ...
  ML split: test
  Mode    : Full
  Eval win: 2024-01-01 -> 2024-12-31

PART A: RULE-BASED
  T1: NN (pair,strategy) tuples survive
  T2 sweep complete
  T3 TP/SL grid complete
  T4: NN configs pass stability filter
  T5 final on locked test split
...
Expected console output (banner separators trimmed):
